    ...
    **kwargs: dict, optional
    ...
    def __init__(
        ...
        **kwargs
        ...
        # Get graph to output module interface parameters 
        self.input_n_atombasis = config.get('input_n_atombasis')
        ...
        if any([
            ...
            # Assign output block to dictionary
            if 'energy' in self.output_properties:
        ...
        for prop in self.output_properties:
            ...
            # Initialize scaling factor and shifting term
    ...
    def set_output_scaling(
        self,
        output_scaling_parameter: Dict[str, torch.Tensor],
    ):
        """
        Update output scaling factor and shift term dictionary
        
        Parameters
        ----------
        output_scaling_parameter: dict(str, torch.Tensor(2))
            Scaling factor and shift term parameter pair (item) for each 
            property (key).

        """
        
        # Assign scaling factor and shift term
        for prop, (scale, shift) in output_scaling_parameter.items():
            self.output_scaling[prop] = (scale, shift)
    ...
    def forward(
        ...
        for prop, (scale, shift) in self.output_scaling.items():
            output_prediction[prop] = (
